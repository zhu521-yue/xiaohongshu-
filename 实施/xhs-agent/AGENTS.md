...
- 最近主线代码已包含 M5 第五片合规留痕与召回解释可见化、召回解释进入 LangGraph 生成上下文、LangGraph M5 smoke 校验增强、可控召回解释 smoke 与节点顺序修复、合规后记忆刷新节点，以及本地 embedding 语义召回基线；本地 `master` 继续领先 `origin/master`，远端同步主要走现有 PR 分支。
