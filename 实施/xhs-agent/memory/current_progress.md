...
## 2026-06-15 项目目标调整为面试展示版两阶段闭环

本轮按用户要求调整项目完成口径：当前目标不再是长期生产化大而全系统，而是“面试展示用、相对稳的完整两阶段版”。后续开发必须继续保持 LangGraph-first，把工作集中在能展示系统架构、业务闭环、记忆增强和阶段二软广/达人能力的主线能力上。

新的完成口径：
- 阶段一主链稳定即可：主题输入 -> 采集/Mock 采集 -> 评论洞察 -> GraphRAG 记忆召回 -> 内容生成 -> 合规 -> 人工审核 -> 私密发布或本地草稿 -> 表现回填 -> 复盘 -> 记忆沉淀。
- 阶段二做最小可演示闭环：商品/卖点输入 -> 软广内容生成 -> 商业合规与频率护栏 -> 达人/平台 mock 匹配 -> 人工审核。
- 保持 LangGraph 为核心编排，不把主线能力绕到 API 旁路。
- 继续保留真实小流量复验能力，但公开发布、定时发布、公开视频、真实千帆/蒲公英全自动、复杂 BI、重型向量数据库、历史大迁移、云部署和长期告警平台全部后置。

后续工作流调整：
- 每轮优先选择能补齐“两阶段演示闭环”的任务，避免继续做前端细节、小工具堆叠或生产化重工程。
- M5 只继续做轻量可解释增强：本地 embedding/GraphRAG 可观测链路、最小 provider 抽象或演示级向量索引即可；复杂图谱可视化和历史大迁移后置。
- 下一条主线优先进入 M6 面试版最小闭环：商品/卖点输入、`product_node.py` 或等价节点、软广生成分支、商业合规/频率护栏、达人/平台 mock 匹配。
- 阶段一收口只补演示必要的 Cookie 提示、健康检查和一次真实小流量复验，不扩展成完整账号运维系统。

当前同步状态：
- 本地最新提交仍是 `d23f2b2 feat: expose semantic recall score summary`。
- 该提交已完成本地测试，但此前推送远端时权限自动审核超时；后续仍需同步到 `codex/m5-rag-eligibility-recall`。
