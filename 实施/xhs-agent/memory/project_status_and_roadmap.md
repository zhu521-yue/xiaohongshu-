...
本文档用于记录当前项目的全局状态、我们期待实现的工程形态，以及尚未实现的关键环节。当前口径已从“长期生产化完整系统”调整为“面试展示用、相对稳的完整两阶段版”；它不记录长测试输出，只记录对后续开发有价值的结论。

## 1. 项目目标

本项目目标是实现一套“面试展示用的小红书两阶段多智能体运营系统”，面向小红书冷启动账号，展示从选题洞察到内容生成、人工审核、私密发布、表现复盘、GraphRAG 记忆沉淀，再到阶段二软广/达人能力的可讲述闭环。当前优先级是相对稳定、链路完整、架构清晰，而不是追求长期生产化、大规模部署或真实平台全量自动化。
...
当前定位：阶段一 MVP 与 LangGraph-first 私密发布闭环已基本站稳，M5 GraphRAG 运营记忆正在增强；后续进入“面试展示版完整两阶段闭环”阶段。生产级高并发、复杂部署、长期告警、公开/定时/视频发布、完整千帆/蒲公英自动化均降为后置增强。

面试展示版完成口径：

1. 阶段一主链必须稳定：主题输入 -> 采集/Mock 采集 -> 评论洞察 -> GraphRAG 记忆召回 -> 内容生成 -> 合规 -> 人工审核 -> 私密发布或本地草稿 -> 表现回填 -> 复盘 -> 记忆沉淀。
2. 阶段二必须可演示：商品/卖点输入 -> 软广内容生成 -> 商业合规与频率护栏 -> 达人/平台 mock 匹配 -> 人工审核。
3. LangGraph 必须是主编排核心；API、脚本和工作台都应服务同一条图内主链。
4. 对面试不必要的长期生产化能力后置：公开发布、定时发布、公开视频、真实千帆/蒲公英全自动、复杂 BI、重型向量数据库、历史大迁移、云部署和长期告警平台。
